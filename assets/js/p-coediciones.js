$(function() {
    ...
    $('#tableCoediciones tbody').on('click', '#linkCoedicion', function() {
        var table = $('#tableCoediciones').DataTable();
        var data = table.row($(this).parents('tr')).data();
        mdAltaEdicion($(this).data('command'));
        loadData(data, $(this).data('command'));
    });

    $('#tableCoediciones tbody').on('click', '#btnEditar', function() {
        var table = $('#tableCoediciones').DataTable();
        var data = table.row($(this).parents('tr')).data();
        mdAltaEdicion($(this).data('command'));
        loadData(data, $(this).data('command'));
    ...
    //   }
    // });
    $('#tableCoediciones').on('page.dt', function() {
        crearAcciones($('#divMenuA').find(".active").data("value"));
    });
    $('#tableCoediciones').on('search.dt', function() {
        crearAcciones($('#divMenuA').find(".active").data("value"));
    });
            ...
            $('#tableCoediciones').DataTable({
                ...
                columns: [{
                    ...
                    mRender: function(data, type, full) {
                        return '<a type="button" id="linkCoedicion"  href="#" data-command="CONSULTA" /><strong>' + data + '</strong></a>';
                    }
                ...
                {
                    mRender: function(data, type, full) {
                        return "<button id='btnEditar' type='button' class='ui violet icon button btnEditar' data-command='EDITAR' /><i class='fa fa-pencil-alt'></i></button>";
                    },
                    width: "8%",
                },
                {
                    mRender: function(data, type, full) {
                        return '<button id="btnEliminar" data-toggle="tooltip" data-placement="left" title="Eliminar" type="button" class="ui red icon button btnEliminar" onclick="deleteCoedicion(' + full.idCoedicion + ')" /><i class="fa fa-trash"></i></button>';
                    },
                    width: "8%",
                }
                ]
...
function saveCoedicion() {

    let idCoedicion = $('#HFIdCoedicion').val() != '' ? $('#HFIdCoedicion').val() : '';
    params = {
        HFCommandName: $('#HFCommandName').val(),
        idCoedicion: idCoedicion,
        nombre: $('#txtNombre').val(),
        autor: $('#txtAutor').val(),
        coautor: $('#txtCoautor').val(),
        objeto: $('#txtDescripcion').val(),
    ...
    fetch('./api/coedicionesApi.php', {
        method: "POST",
        body: JSON.stringify({ 'saveCoedicion': params }),
        dataType: "JSON"
    })
        .then(response => {
            return response.json();
        ...
        .then(response => {
            ...
            $('body').toast({
                class: 'success',
                message: `Coedicion guardado correctamente !`
            });
        }).catch(error => {
...
function loadData(data) {
    ...
    $('#HFIdCoedicion').val(data.id);
    $('#txtNombre').val(data.nombre);
    $('#txtAutor').val(data.autor);
    $('#txtCoautor').val(data.coautor);
    $('#txtDescripcion').val(data.objeto);
    ...
    // $('#formCoediciones :input")').addClass("edited");
}

function deleteCoedicion(idCoedicion) {
    $('#domtoastactions')
        .toast({
            displayTime: 0,
            onDeny: function() {
            ...
            onApprove: function() {
                fetch('./api/coedicionesApi.php', {
                    method: "POST",
                    body: JSON.stringify({ 'deleteCoedicion': { idCoedicion: idCoedicion } }),
                    dataType: "JSON"
                })
                    .then(response => {
                        return response.json();
                    ...
                    .then(response => {
                        ...
                        $('body').toast({
                            class: 'success',
                            message: 'Coedicion eliminado correctamente !'
                        });
                    }).catch(error => {
                        $('body').toast({ class: 'error', message: `An error occured !` + error.message });
                    });
